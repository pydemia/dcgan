...
def daily_sliding_window_extractor(
    ...
    for day, group in grouped:
        ...
        if not group.shape[0]:
            ...
            train_size = int(group.shape[0] * (1 - test_ratio))
...
# %%
col
col_real_range_dict[col]
col_real_range_dict[col][vendor_name]
...
aa.hour.unique()
bb = pd.to_datetime(data['EVT_DTM'], format='%Y%m%d%H%M%S')
data.set_index(bb, inplace=True)

cc = data.index

cc.hour.unique()

selected.index.hour.unique()
